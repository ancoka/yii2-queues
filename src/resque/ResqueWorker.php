...
use queues\resque\job\ResqueJobDirtyExitException;
...
class ResqueWorker
{
        ...
        while (true) {
            ...
            if (!$this->paused) {
                if($blocking === true) {
                    ...
                    $this->updateProcLine('Waiting for ' . implode(',', $this->queues) .
                        ' with blocking timeout ' . $interval);
                } else {
                    $this->updateProcLine('Waiting for ' . implode(',', $this->queues) .
            ...
            if (!$job) {
                ...
                if ($blocking === false) {
                    ...
                    $this->logger->log(LogLevel::INFO,
                        'Sleeping for {interval}', array('interval' => $interval));
                    if ($this->paused) {
            ...
            $this->logger->log(LogLevel::NOTICE,
                'Starting work on {job}', array('job' => $job));
            ...
            if ($this->child > 0) {
                ...
                if ($exitStatus !== 0) {
                    $job->fail(new ResqueJobDirtyExitException(
    ...
    public function perform(ResqueJob $job)
    {
        try {
            ...
            $this->logger->log(LogLevel::CRITICAL,
                '{job} has failed {stack}', array('job' => $job, 'stack' => $e->getMessage()));
            $job->fail($e);
            return;
        }

        $job->updateStatus(ResqueJobStatus::STATUS_COMPLETE);
     ...
     * @param  bool            $blocking
     * @param  int             $timeout
     * @return object|boolean  Instance of ResqueJob if a job is found, false if not.
     */
    ...
    public function killChild()
    {
        ...
        $this->logger->log(LogLevel::INFO,
            'Killing child at {child}', array('child' => $this->child));
        if (exec('ps -o pid,state -p ' . $this->child, $output, $returnCode) && $returnCode != 1) {
            $this->logger->log(LogLevel::DEBUG,
                'Child {child} found, killing.', array('child' => $this->child));
            posix_kill($this->child, SIGKILL);
            $this->child = null;
        } else {
            $this->logger->log(LogLevel::INFO,
                'Child {child} not found, restarting.', array('child' => $this->child));
            $this->shutdown();
        }
    }
    ...
    public function pruneDeadWorkers()
    {
        ...
        foreach($workers as $worker) {
            if (is_object($worker)) {
                ...
                $this->logger->log(LogLevel::INFO,
                    'Pruning dead worker: {worker}', array('worker' => (string)$worker));
    ...
    public function unregisterWorker()
    {
        if(is_object($this->currentJob)) {
            $this->currentJob->fail(new ResqueJobDirtyExitException);
        }
     ...
     *
     * @param object $job ResqueJob instance containing the job we're working on.
     */
    public function workingOn(ResqueJob $job)
    {
        $job->worker = $this;
        $this->currentJob = $job;
        $job->updateStatus(ResqueJobStatus::STATUS_RUNNING);
    ...
    public function doneWorking()
    {
        ...
        ResqueStat::incr('processed:' . (string)$this);
    ...
    {
        return ResqueStat::get($stat . ':' . $this);
    }
